...
function loadAndAddGLBModel(filePath, position, scale, rotation, scene) {
  loaderGLB.load(filePath, function (gltf) {
    ...
    // === ESTA LÍNEA ES CLAVE: marcamos el modelo para reconocerlo después ===
    ...
    // Para que todos los meshes del modelo reciban clics
...
function performRaycastClick() {
  ...
  // Importante: hacer recursive: true para detectar hijos del GLTF
  const intersects = raycaster.intersectObjects(scene.children, true);

  if (intersects.length > 0) {
    // Buscamos el objeto raíz del modelo (el que añadimos con loadAndAddGLBModel)
    let clickedObject = intersects[0].object;
    while (clickedObject.parent && !clickedObject.userData.isGLTFModel) {
      clickedObject = clickedObject.parent;
    }
    const rootModel = clickedObject.userData.isGLTFModel ? clickedObject : null;

    // Si hicimos clic en el avión verde (o cualquier modelo GLTF que tenga esta marca)
    if (rootModel) {
      if (selectedModel === rootModel) {
        // Si ya estaba seleccionado → parar rotación
        isRotating = !isRotating;
      } else {
        // Si es un modelo nuevo → parar el anterior y empezar este
        selectedModel = rootModel;
        isRotating = true;
      }
      console.log(isRotating ? "Rotación activada" : "Rotación detenida");
    }

    // Feedback visual (opcional): cambia color del cubo si tocas el cubo
    if (clickedObject === cube || clickedObject.parent === cube) {
...
function animate() {
  // === ROTACIÓN DEL MODELO SELECCIONADO ===
  if (isRotating && selectedModel) {
    selectedModel.rotation.y += 0.01;  // velocidad suave de giro
  }
